...
private:
   ...
   Fraction *data;
public:
   int size() const {return this->sizeF;};
   int capacity() const { return  this->capacityF;};

   void setSize(int size){this->sizeF = size;};
   void setCapcity(int capacity){this->capacityF = capacity;};
   void setData(Fraction* data){this->data = data;};
   Fraction * getData(){return this->data;};
   Vector(int size = 0);
    Vector (Vector &&carryVector);
    Vector (const Vector &copyVector);
    ~Vector();
    Vector& operator=(const Vector &copyVector);
    Vector& operator=(Vector &&diffVector);
